...
interface XmlCompareProps {
    xml1: string;
    xml2: string;
}

/**
 * Side-by-side XML comparison with colorized differences and synchronized scrolling
 */
const XmlCompare: React.FC<XmlCompareProps> = ({ xml1, xml2 }) => {
    ...
    // Parse XML to lines
    const xmlToLines = (xml: string): string[] => {
        // Basic beautification for comparison
        const formatted = xml
            .replace(/>\s*</g, '>\n<')
            .replace(/>\s*([^<]+)\s*</g, '>$1<')
            .trim();
    ...
    const leftLines = useMemo(() => xmlToLines(xml1), [xml1]);
    const rightLines = useMemo(() => xmlToLines(xml2), [xml2]);
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < lines1.length; i++) {
            for (let j = 0; j < lines2.length; j++) {
                if (!matched2.has(j) && lines1[i] === lines2[j]) {
                    matched1.add(i);
                    matched2.add(j);
                    break;
                }
        ...
        setDiff(result);
    }, [xml1, xml2, leftLines, rightLines]);
    ...
    const removedLines = useMemo(() => new Set(diff.removed.map(d => d.value)), [diff.removed]);
    const addedLines = useMemo(() => new Set(diff.added.map(d => d.value)), [diff.added]);

    // Get line style based on diff
    const getLineStyle = (line: string, side: 'left' | 'right'): string => {
        const isDark = themeMode === 'dark';
        if (side === 'left' && removedLines.has(line)) {
            return isDark ? 'bg-red-500/20 text-red-300' : 'bg-red-100 text-red-700';
        }
        if (side === 'right' && addedLines.has(line)) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className={`rounded-xl border ${borderClass} overflow-hidden`}>
                    <div className={`px-4 py-2 ${bgClass} border-b ${borderClass} flex items-center gap-2`}>
                        ...
                        <span className={`text-sm font-semibold ${textClass}`}>XML 1</span>
                    </div>
                    <div
                        ref={leftScrollRef}
                ...
                <div className={`rounded-xl border ${borderClass} overflow-hidden`}>
                    <div className={`px-4 py-2 ${bgClass} border-b ${borderClass} flex items-center gap-2`}>
                        ...
                        <span className={`text-sm font-semibold ${textClass}`}>XML 2</span>
                    </div>
                    <div
                        ref={rightScrollRef}
